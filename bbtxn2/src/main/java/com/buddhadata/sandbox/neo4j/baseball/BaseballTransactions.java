...
public class BaseballTransactions {
    ...
    /**
     * Singleton unassigned node used everytime a player is not a member of a team.
     */
    private final Unassigned unassigned = new Unassigned();
    ...
    /**
     * Saves the singleton Unassigned node to the database
     * @param session Neo4J database session
     */
    private void saveUnassigned (Session session) {

        try (Transaction txn = session.beginTransaction()) {

            //  Save the unassigned node
            session.save (unassigned);
            ...
            while (br.ready()) {
                ...
                switch (type) {
                    ...
                    case Xr: // returned to original team after expansion draft
                        FromToTransactionProcessor.instance.process(session, type, fields, unassigned);
                        break;
                    ...
                    case Z:  // voluntarily retired
                        FromTransactionProcessor.instance.process(session, type, fields, unassigned);
                        break;
                    ...
                    case Zr: // returned from voluntarily retired list
                        ToTransactionProcessor.instance.process(session, type, fields, unassigned);
                        break;
